...
class_0 = data.large[data.large['diabetes'] == 0,]
class_1 = data.large[data.large['diabetes'] == 1,]
class_1_over = class_1[sample(nrow(class_1), nrow(class_0), replace = TRUE), ]
data.large.sampled = rbind(class_1_over, class_0)

# Split over-sampled data into training and test
sample.large.sampled <- sample(c(TRUE, FALSE), nrow(data.large.sampled), replace=TRUE, prob=c(0.7,0.3))
train.large.sampled  <- data.large.sampled[sample.large.sampled, ]
test.large.sampled   <- data.large.sampled[!sample.large.sampled, ]
nrow(data.large.sampled)
nrow(train.large.sampled)
nrow(test.large.sampled)
```
...
MISLEADING RESULTS: With over-sampling, test set will contain entries from 
training so cannot rely on hold-out predictions.
...
# test.large.nodup <- test.large.sampled[!duplicated(test.large.sampled), ]
# yhat.rf.large.nodup = predict(rf.large.sampled, newdata = test.large.nodup)
# # Confusion Matrix
# (cm.large.nodup = confusionMatrix(test.large.nodup$diabetes, yhat.rf.large.nodup))
# # Variable Analysis
# importance(rf.large.sampled)
# varImpPlot(rf.large.sampled)
```

(With sampling, test set removed duplicates):
...
False negative: 0.064
False positive: 0.014
True negative: 0.5418
True positive: 0.314


```{r}
# set.seed(1)
# yhat.rf.large.wdup = predict(rf.large.sampled, newdata = test.large.sampled)
# # Confusion Matrix
# (cm.large.wdup = confusionMatrix(test.large.sampled$diabetes, yhat.rf.large.wdup))
```

(With sampling, test set containing duplicates):
